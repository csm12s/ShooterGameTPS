...
#pragma endregion
	...
	MoveComp->MaxWalkSpeedCrouched = 300;
	// enable crouch
	// todo crouch mid air
	...
	WalkingSpeedModifier = 0.5f;//todo
...
void AShooterCharacter::UpdatePawnMeshes()
{
	...
	//GetMesh()->SetVisibility(!bFirstPerson, true);
...
void AShooterCharacter::SetSprinting(bool bNewSprinting, bool bToggle = false)
{
	bWantsToSprint = bNewSprinting;
	bWantsToSprintToggled = bNewSprinting && bToggle;

	if (bWantsToSprint && bIsCrouched) {
		UnCrouch();
	}

	if (GetLocalRole() < ROLE_Authority)
	{
		ServerSetSprinting(bNewSprinting, bToggle);
	}
}

bool AShooterCharacter::ServerSetSprinting_Validate(bool bNewRunning, bool bToggle)
{
	return true;
}

void AShooterCharacter::ServerSetSprinting_Implementation(bool bNewRunning, bool bToggle)
{
	SetSprinting(bNewRunning, bToggle);
}

void AShooterCharacter::UpdateRunSounds()
{
	const bool bIsRunSoundPlaying = RunLoopAC != nullptr && RunLoopAC->IsActive();
	const bool bWantsRunSoundPlaying = IsSprinting() && IsMoving();
...
void AShooterCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	// Aim
	PlayerInputComponent->BindAction("Targeting", IE_Pressed, this, &AShooterCharacter::OnSwitchTargeting);
	
	PlayerInputComponent->BindAction("SwitchView", IE_Pressed, this, &AShooterCharacter::ToggleView);
	...
	PlayerInputComponent->BindAction("CrouchToggle", IE_Pressed, this, &AShooterCharacter::OnCrouchToggle);

	// sprint
	// todo rename
	PlayerInputComponent->BindAction("RunToggle", IE_Pressed, this, &AShooterCharacter::OnStartRunningToggle);
	PlayerInputComponent->BindAction("Run", IE_Pressed, this, &AShooterCharacter::OnStartRunning);
	PlayerInputComponent->BindAction("Run", IE_Released, this, &AShooterCharacter::OnStopSprinting);
}
...
void AShooterCharacter::HandleFireInput()
{
	AShooterPlayerController* MyPC = Cast<AShooterPlayerController>(Controller);
...
void AShooterCharacter::OnSwitchTargeting() 
{
	if (IsTargeting()) 
	{
		OnStopTargeting();
...
void AShooterCharacter::OnNextWeapon()
{
	...
	if (MyPC && MyPC->IsGameInputAllowed())
	{
		if (Inventory.Num() >= 2 && (CurrentWeapon == NULL || CurrentWeapon->GetCurrentState() != EWeaponState::Drawing))
		{
...
void AShooterCharacter::OnStopSprinting()
{
	SetSprinting(false, false);
}
...
bool AShooterCharacter::IsSprinting() const
{
	...
	return (bWantsToSprint || bWantsToSprintToggled) 
		&& !GetVelocity().IsZero() 
...
void AShooterCharacter::Tick(float DeltaSeconds)
{
	...
	if (bWantsToSprintToggled && !IsSprinting())
	{
		SetSprinting(false, false);
	}
	AShooterPlayerController* MyPC = Cast<AShooterPlayerController>(Controller);
...
bool AShooterCharacter::IsFirstPerson() const
{
	// todo ref, is playing
	return IsAlive() 
		&& Controller && Controller->IsLocalPlayerController()
...
void AShooterCharacter::InitView()
{
	APlayerController* PC = Cast<APlayerController>(GetController());

	if (PC && PC->IsLocalController())
	{
		bUseFirstPerson = bStartInFirstPerson;
		SetView(bUseFirstPerson);
	}
}
...
void AShooterCharacter::StartFire()
{
	...
	if (IsLocallyControlled()
		&& CurrentWeapon && CurrentWeapon->GetCurrentAmmo() == 0)
	{
		UGameplayStatics::SpawnSoundAttached(CurrentWeapon->OutOfAmmoSound,
			GetRootComponent());
...
void AShooterCharacter::SetView(bool isFirstPerson)
{
	AShooterPlayerController* PC = GetController<AShooterPlayerController>();
...
void AShooterCharacter::OnCrouchToggle()
{
	if (IsSprinting())
	{
		SetSprinting(false);
